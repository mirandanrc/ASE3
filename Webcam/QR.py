...
#Variable para siempre centrar QR
#def centrar(center_x, X):
    center_x = 680 / 2
    #center_y = 480 / 2
    if (center_x == X):
...
if __name__=="__main__":
  ...
  while (True):
    ...
    frame = cv2.resize(frame, (680, 480)) #Cambiar el tamaño de la ventana que despliega
    ...
    if len(points) > 0:
      ...
      for (markerCorner, markerID) in zip(points, ids):
        ...
        #Calculamos el centro de nuestra pantalla
          ...
          #Le damos la direccion que queremos al robot
          #distancia_x = camera_centerx - X
          #distancia_y = camera_centery - Y
